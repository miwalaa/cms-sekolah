...
type FooterClientProps = {
  ...
  recentPosts: Array<Pick<Post, 'title' | 'slug' | 'publishedAt' | 'meta' | 'heroImage'>>
  contactInfo: {
    address?: string | null
    phone?: string | null
    email?: string | null
...
export function FooterClient({ navItems, recentPosts, contactInfo }: FooterClientProps) {
  const CONTACT = {
  ...
  // Count visible sections (excluding Logo & About which is always visible)
  const visibleSectionsCount = [hasContactInfo, hasRecentPosts, hasNavItems].filter(Boolean).length

  // Determine grid columns based on visible sections
  const totalColumns = 1 + visibleSectionsCount
  const gridColsClass =
    totalColumns === 1
      ? 'lg:grid-cols-1'
      : totalColumns === 2
  ...
  return (
    <footer className="mt-auto bg-brand/95 text-white">
      {/* Top Section */}
      <motion.div
        className="container px-4 py-16 md:py-[65px] pt-24"
        initial={{ opacity: 0, y: 20 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true }}
        transition={{ duration: 0.6, ease: 'easeOut' }}
      >
        <div className={`grid grid-cols-1 gap-10 md:grid-cols-2 ${gridColsClass}`}>
