...
const ContactForm: React.FC<ContactFormProps> = ({
  ...
  const getStatusMessage = (): string => {
    if (hasSubmitted) return 'Terima kasih! Pesan Anda telah berhasil dikirim dan akan segera kami proses.'
      ...
      <form onSubmit={onSubmit} className="space-y-4" noValidate>
        ...
        <div>
          <button
            type="submit"
            disabled={isLoading}
            className="inline-flex items-center justify-center rounded-lg bg-primary px-5 py-2.5 font-medium text-white shadow-sm hover:opacity-95 disabled:opacity-75 disabled:cursor-not-allowed"
          >
            {isLoading ? (
              <>
